		...
		async function getQuota() {
			try {
				const result = await axios.get(
					`${process.env.NEXT_PUBLIC_APP_PROTOCOL}://${process.env.NEXT_PUBLIC_APP_HOST}:${process.env.NEXT_PUBLIC_APP_PORT}/api/quota`
				)

				const { KU1, KU2, KU3, KU4, KU5 } = result.data

				if (KU1 === 0 && KU2 === 0) {
					alert('Maaf kuota ibadah Jumat Agung sudah penuh')
				} else if (KU3 === 0 && KU4 === 0 && KU5 === 0) {
					alert('Maaf kuota ibadah Paskah sudah penuh')
				} else {
					...
					<CardContent className='space-y-3 pt-6'>
						...
						<div className='text-center text-[#10133a] space-y-1'>
							<h1 className='text-xl font-semibold text-balance leading-tight'>
								{`Formulir Registrasi Online Ibadah ${
									query == 'j'
										? 'Jumat Agung'
										: query == 'p' && 'Paskah'
								}`}
